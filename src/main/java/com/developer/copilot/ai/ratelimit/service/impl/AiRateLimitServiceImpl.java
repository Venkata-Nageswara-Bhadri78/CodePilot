...
public class AiRateLimitServiceImpl implements AiRateLimitService {
    ...
    private final AiRedisService redisService;
    private final ConcurrentHashMap<String, Deque<Long>> hitWindows = new ConcurrentHashMap<>();

    public AiRateLimitServiceImpl(AiRedisService redisService) {
    ...
    public RateLimitResult consume(String bucket, String identity, int limit, long windowSeconds) {
        ...
        if (redisService != null) {
            try {
                ...
                log.warn("AI Redis rate-limit failed; using in-memory fallback: {}", ex.getMessage());
            }
        }
